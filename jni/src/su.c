...
static int from_init(struct su_initiator *from) {
    ...
    if (argv_rest) {
        strncpy(from->args, argv_rest, sizeof(from->args) - 1);
    ...
    if (pw && pw->pw_name) {
        strncpy(from->name, pw->pw_name, sizeof(from->name) - 1);
    }
...
int su_main(int argc, char *argv[], int need_client) {
    ...
    if (optind < argc && strcmp(argv[optind], "--")) {
        ...
        if (!pw) {
            ...
            if (pw->pw_name)
                strncpy(ctx.to.name, pw->pw_name, sizeof(ctx.to.name) - 1);
        }
        optind++;
    }
    if (optind < argc && !strcmp(argv[optind], "--")) {
