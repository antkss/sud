...
int setup_sock_daemon(const char *path_abs, size_t size) {
	...
	memcpy(sun.sun_path, path_abs, size);
    if (fd < 0) {
        PLOGE("socket");
        return -1;
    }
...
#define SUPERUSER_SOCK "SUPERUSER"
#define PEKA_SOCK_PATH "PEKAPATH"
int run_daemon() {
    int fd = setup_sock_daemon("\0" SUPERUSER_SOCK, sizeof(SUPERUSER_SOCK) + 1);
	int path_fd = setup_sock_daemon("\0" PEKA_SOCK_PATH, sizeof(PEKA_SOCK_PATH) + 1);
    int client;
	int client_path;
    while ((client = accept(fd, NULL, NULL)) > 0 && (client_path = accept(path_fd, NULL, NULL)) > 0) {
        if (fork_zero_fucks() == 0) {
			char path[PATH_MAX] = {0};
            close(fd);
			close(path_fd);
			int num = recv(client_path, path, sizeof(path), 0);
			if (num < 0) {
				LOGE("recv error");
			} else {
				path[num] = 0;
...
int setup_sock_connect(const char *path_abs, size_t size) {
    struct sockaddr_un sun;
...
int connect_daemon(int argc, char *argv[], int ppid) {
	...
	int sockfd = setup_sock_connect("\0" SUPERUSER_SOCK, sizeof(SUPERUSER_SOCK));
	int path_sockfd = setup_sock_connect("\0" PEKA_SOCK_PATH, sizeof(PEKA_SOCK_PATH));
	if (!getcwd(req_path, sizeof(req_path))) strncpy(req_path, ".", sizeof(req_path)-1);
	send(path_sockfd, req_path, strlen(req_path), 0);
    ...
    write_string(sockfd, pts_slave);
    // User ID
    write_int(sockfd, uid);
    // Parent PID
